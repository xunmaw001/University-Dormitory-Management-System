...
<!--/Pre Loader -->
                ...
                <div class="row breadcrumb-bar">
                    ...
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">缺勤列表</li>
                        </ol>
                    </div>
                </div>
            </div>
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        ...
                                        <div class="layui-input-inline">
                                            <input type="text" id="queqinTimeSearch" style="width: 140px;" class="form-control form-control-sm"
                                                   placeholder="缺勤日期" aria-controls="tableId">
                                        </div>
                                    </div>
    ...
    <script>
        ...
        <!-- 级联表的级联字典表 -->
            ...
            //缺勤日期
            var queqinTimeSearchInput = $('#queqinTimeSearch');
            if( queqinTimeSearchInput != null){
                if (queqinTimeSearchInput.val() != null && queqinTimeSearchInput.val() != '') {
                    searchForm.queqinTime = $('#queqinTimeSearch').val();
                }
            }
                        getDataList();
        }
        ...
        function getDataList() {
            http("queqin/page", "GET", {
                page: pageIndex,
                limit: pageSize,
                sort: sortColumn,
                order: sortOrder,
                //本表的
                queqinTime: searchForm.queqinTime,
            //级联表的
        ...
        function setDataRow(item, number) {
            ...
            //缺勤日期
            var queqinTimeCell = document.createElement('td');
            queqinTimeCell.innerHTML = item.queqinTime;
            row.appendChild(queqinTimeCell);


                //备注
            var queqinContentCell = document.createElement('td');
            //如果图文详情包含图片
            if (item.queqinContent && item.queqinContent.indexOf('img') != -1) {
                //暂时只考虑图片+文字 和 文字+图片的情况
                var beginIndex = item.queqinContent.indexOf('<img');
                var endIndex = item.queqinContent.indexOf('>');
                    queqinContentCell.innerHTML = item.queqinContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
            } else {
                if (item.queqinContent != null && item.queqinContent != "" && item.queqinContent.length >= 11) {
                        queqinContentCell.innerHTML = item.queqinContent.substring(0, 10) + "...";
                } else {
                        queqinContentCell.innerHTML = item.queqinContent;
                }
            }
            row.appendChild(queqinContentCell);
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("queqin/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addqueqin", "addqueqin");
            window.location.href = "add-or-update.jsp"
        }
